...
public class DeviceManager : IDisposable
{
    ...
    private Dictionary<string, UsbDeviceInfo> _connectedDevices = new();
    ...
    private void PollDevices()
    {
        ...
        // Get all Apple devices - same as AnyMiro
        ...
        foreach (var device in deviceList)
        {
            ...
            lock (_lock)
            {
                ...
                if (!_connectedDevices.ContainsKey(udid))
                {
                    ...
                    Log($"Device connected: {deviceInfo.DisplayName}");
        ...
        lock (_lock)
        {
            ...
            foreach (var udid in toRemove)
            {
                ...
                Log($"Device disconnected: {deviceInfo.DisplayName}");
    ...
    {
        var name = device.Product != null ? device.Product : "Unknown iOS Device";
    ...
    public UsbEndpointWriter? ConnectToDevice(string udid)
    {
        lock (_lock)
        {
            if (!_connectedDevices.TryGetValue(udid, out var deviceInfo))
                return null;
            
            // Open device
            var device = _usbContext?.OpenDevice(deviceInfo.VendorId, deviceInfo.ProductId);
            ...
            // Get bulk endpoints - same as AnyMiro
    ...
    private static void Log(string message)
    {
        try
        {
            File.AppendAllText("mirance.log", 
                $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] [DeviceManager] {message}\n");
        }
        catch { }
    }
}
...
/// <summary>
/// USB Device Information - same structure as AnyMiro
/// </summary>
